...
var isReact19 = Number("19.3.0".split(".")[0]) >= 19;
		...
		if (pointer) {
			if (activePointer.current === event.pointerId) {
				...
				if (gesture.current === Gesture.SWIPE) {
					if (Math.abs(currentOffset) > .3 * containerWidth || Math.abs(currentOffset) > 5 && duration < swipeAnimationDuration) onSwipeFinish(currentOffset, duration);
					else onSwipeCancel(currentOffset);
				} else if (gesture.current === Gesture.PULL) {
					if (exceedsPullThreshold(currentOffset, 60)) onPullFinish(currentOffset, duration);
